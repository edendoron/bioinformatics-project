...
```{r loading data}
data <- read.csv('./E-MTAB-2758-raw-counts.tsv', header = T, sep = '\t', row.names = 'Gene.ID')

data_matrix <- as.matrix(data)
gene_map <- data_matrix[ ,c(1)]
raw_data <- data_matrix[, -1]

experiment_design_data <- read.csv('./E-MTAB-2758-experiment-design.tsv', header = T, sep = '\t', row.names = 1, stringsAsFactors = T)
...
legend("bottomright",
       legend = c("Control", "Osmium Based Compound"),
       fill = 1:2, bty = 'n', ncol = 3,
       cex = 0.5)
...
legend("bottomright",
       legend = c("0 hour", "4 hour", "12 hour", "24 hour", "48 hour"),
       fill = 1:5,
       cex = 0.5)
